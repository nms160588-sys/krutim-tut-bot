...
DESIGN_PHOTOS = [
    ("1_kompakt.png", "Компактная точка навынос — лофт, дерево и бетон, фирменный логотип"),
    ("2_organic.png", "Большой формат — витрина ORGANIC с фруктами, кофе-зона, мороженое"),
    ("3_posadka.png", "Формат с посадкой Крутим Жарим — диваны, барная стойка, лаунж"),
]
...
# Что бот предлагает дальше после закрытия чек-листа (ведение по этапам)
NEXT_STEP = {
    "loc": "Локацию закрыли. Дальше дизайн — напиши дизайн, покажу три формата точек и подберём твой.",
    "hire": "Люди есть. Дальше финальная подготовка — напиши открытие, дам чек-лист перед стартом.",
    "open": "Всё готово к старту. Назначай дату, в первые дни раздай листовки и держи акцию на первый заказ.",
}
...
def send_design(chat_id):
    send_msg(chat_id, "Дизайн точки делаем мы под твоё помещение, своего дизайнера искать не надо. "
                      "Есть три готовых формата, смотри как это выглядит вживую:")
    for fname, cap in DESIGN_PHOTOS:
        send_photo(chat_id, os.path.join(ASSETS, fname), cap)
    send_msg(chat_id, "Последовательность важная: сначала локация, под неё мы рисуем дизайн зала и вывески, "
                      "потом под дизайн закупаем оборудование и только затем ремонт. Так ничего не переделываешь. "
                      "Стиль фирменный: бетон и дерево, керамогранит, рейки, фирменный логотип и надпись ALL NATURAL SHAURMA. "
                      "Все документы и планы тут: " + DESIGN_FOLDER_URL + " "
                      "Пришли план помещения с размерами — адаптируем макет под тебя. В каком городе помещение?")

def send_plans(chat_id):
    send_msg(chat_id, "Под помещение мы готовим полный пакет чертежей: план мебели, план розеток, план освещения "
                      "и план отделки со спецификацией материалов. Их отдаёшь бригаде на ремонт. Вот как выглядят планы:")
    send_photo(chat_id, os.path.join(ASSETS, "plan_mebeli.png"), "План мебели — размеры рабочих столов и зон")
    send_photo(chat_id, os.path.join(ASSETS, "plan_rozetok.png"), "План розеток — высоты и точки подключения")
    send_msg(chat_id, "Это пример. Полный комплект под твою площадь сделаем после замеров. "
                      "Пришли план помещения с размерами и фото — и запустим проект. Замеры уже делал?")
...
def handle_callback(cq):
    ...
    if len(s) == len(CHECKLISTS[key]["items"]) and (chat_id, key) not in done_notified:
        ...
        if key in NEXT_STEP:
            send_msg(chat_id, NEXT_STEP[key])  # сам ведём к следующему этапу
