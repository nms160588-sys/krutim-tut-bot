...
def instant_answer(t):
    ...
    if any(w in t for w in ["ремонт", "своими рук", "бригад", "кто делает рем", "ремонтн"]):
        return ("Ремонт делаешь сам — нанимаешь местную бригаду по нашему техзаданию: зонирование зала и рабочей зоны, "
                "точки воды и канализации, электрика на 15 кВт, место под вытяжку, отделка по брендбуку. "
                "Мы даём готовый дизайн-проект с планами мебели, розеток, освещения и отделки — напиши планы, пришлю. "
                "Сложного дизайна не надо, главное чисто и по стандарту.")
    if any(w in t for w in ["вода", "канализац", "водоснабж", "сантехник", "слив", "мойка подключ"]):
        return ("Нужна холодная и горячая вода и канализация в рабочей зоне под мойку и раковину. "
                "Если подведено не везде — бригада разводит трубы при ремонте, точки воды есть в плане мебели. "
                "Заранее проверь, что в помещении вообще есть водоснабжение и слив, это критично для общепита.")
    if any(w in t for w in ["вентиляц", "вытяжк", "запах", "воздух"]):
        return ("Вытяжка обязательна — работаешь с грилем и фритюром, без неё СЭС не пропустит и запах пойдёт к соседям. "
                "Закладывай зонт над грилем и вывод вентиляции по проекту. Место под вытяжку отмечено в планах. "
                "Уточни у арендодателя, можно ли вывести вентиляцию на фасад или крышу.")
    ...
    if any(w in t for w in ["отделк", "материал", "стен", "пол ", "керамогранит", "профнастил", "штукатур"]):
    ...
    if any(w in t for w in ["график", "часы работ", "режим работ", "во сколько", "смена скольк", "график работ"]):
