...
def instant_answer(t):
    ...
    if any(w in t for w in ["ремонт", "отделк", "своими рук", "бригад", "кто делает рем"]):
        return ("Ремонт делаешь сам — нанимаешь местную бригаду, так дешевле и быстрее, чем под ключ. "
                ...
                "Сложного дизайна не надо, главное чисто и по брендбуку. Помещение уже нашёл или ещё подбираешь?")
    if any(w in t for w in ["дизайн", "брендбук", "оформлен", "макет", "кто рисует", "интерьер"]):
        return ("Дизайн делаем мы. У тебя готовый брендбук Крутим Тут — зал, вывеска, меню-борд в фирменном стиле. "
...
def hook():
    ...
    if "callback_query" in u:
        handle_callback(u["callback_query"]); return "ok"
    msg = u.get("message", {})
